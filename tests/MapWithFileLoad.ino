...
//As of now it loads duration, arrow layout and nname (which I don't use for anything and probably won't)
//IMPORTANT: Don't forget that I still need to add music frequencies and durationns, might add these 2 attributes when the first song is finished.
...
void parseMapFile(char* filename) {
  File file = SD.open(filename, FILE_READ);
  
  //---Name----
  String mapName = file.readStringUntil('\n');
  mapName.trim();
  ...
  arrowNumStr.trim();
  int arrowNum = arrowNumStr.toInt();
  if (arrowNum > MAX_ARROWS) arrowNum = MAX_ARROWS;
  Serial.println(arrowNum);
  ...
  for (int i = 0; i < arrowNum; i++) {
    ...
    Serial.println("Raw Arrow Line: [" + line + "]");  // debug
    char type;
    unsigned long startTime;
    unsigned long endTime;
    sscanf(line.c_str(), "%c,%lu,%lu", &type, &startTime, &endTime);

    int type_parse;

    switch(type){ 
      case 'L': type_parse = LEFT; break; 
      case 'U': type_parse = UP; break; 
      case 'D': type_parse = DOWN; break; 
      case 'R': type_parse = RIGHT; break; 
    }

    Serial.print(type);Serial.print(" | ");Serial.print(startTime);Serial.print(" | ");Serial.println(endTime);
